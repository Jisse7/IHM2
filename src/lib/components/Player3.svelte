<script>
  ...
  import { audioPlayer, status, isPlaying, index, thisNextTrack, currentPlaylist} from '$lib/playlists.js';
...
import {volume} from '$lib/stores.js'
  ...
  $: {
      if ($currentPlaylist) {
          playlist = getPlaylistByName($currentPlaylist) || [];
      }
      if ($thisNextTrack !== undefined) {
          currentIndex = $thisNextTrack;
      }
  }
  ...
  $: console.log('Current Track:', playlist[currentIndex]);
  ...
  function playNext() {
      ...
      thisNextTrack.set(nextIndex); // Met à jour le store
      loadTrack(nextIndex);
      audio.play().then(() => {
          isPlaying.set(true);
          status.set('playing');
  ...
  function playPrevious() {
      ...
      thisNextTrack.set(prevIndex); // Met à jour le store
      loadTrack(prevIndex);
      audio.play().then(() => {
          isPlaying.set(true);
          status.set('playing');
  ...
  // pour changer la position de lecture via la barre de progression
  ...
  // pour init les événements audio
  onMount(() => {
      audio = new Audio();
      ...
      audio.addEventListener('ended', () => {
          playNext(); // Passer automatiquement à la piste suivante
      });
