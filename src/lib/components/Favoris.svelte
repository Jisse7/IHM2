<script>
  ...
  import { favoris, toggleFavori, thisNextTrack, chansonSwap,currentPlaylist } from '$lib/store_favoris';
  ...
  let thisPlaylist= "favoris";


  function playThisNextTrack(thisTrackIndex, playlistName) {
...
<div class="spotify-playlists">
  <strong><h2>Favoris</h2></strong>
  <div class="list">
    {#each $favoris as musique, index}
      <div class="item">
        ...
        <div class="play">
          ...
          <a href={'#'} on:click|preventDefault={() => playThisNextTrack(index, thisPlaylist)}>
        ...
        <!-- Affichage des playlists contenant ce morceau -->
...
{#if $chansonSwap && ($thisNextTrack !== null || $thisNextTrack === 0)}
  <Player />
{/if}
  <!-- <button on:click={deBug}>DEBUGGGGGG</button> -->
...
.playlist-info {
  ...
  .spotify-playlists .list .item .play {
    ...
    transform: translateY(-50%); /*POUR AJUSTER VERTICALEMNT */
    padding: 10px;
    background-color: #1db954;
    border-radius: 50%;
    display: flex;
    ...
    opacity: 0; /* ON CACHE PAR DEFAUT */
    transition: opacity ease 0.4s;
    /* pointer-events: none;  */
    /* POUR QUE CA BLOQUE PAS LES INTERACTIONS */
}

.spotify-playlists .list .item:hover img + .play {
    opacity: 1; /* Apparaît uniquement au survol de l'image */
}
...
.spotify-playlists .list .item .play .fa {
  ...
  .spotify-playlists .list .item .add-to-playlist {
    position: absolute;
    right: 10px;
    bottom: 10px;
    cursor: pointer;
  ...
  .spotify-playlists .list .item .add-to-playlist .fa {
    font-size: 20px;
    color: #ccc;
    transition: all 0.3s;
  }

  .spotify-playlists .list .item .add-to-playlist .fa:hover {
    color: #1db954;
  }
